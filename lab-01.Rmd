...
visitors %>% count()
```
There are 348 attractions
```{r}
arrange(visitors,desc(n_2022))
filter(visitors,attraction == 'National Museum of Scotland')
filter(visitors,region == 'Yorkshire and the Humber',setting == 'O',admission == 'Members',n_2022>=100000)
```
...
visitors %>% 
  summarise(
    mean2021 = mean(n_2021,na.rm = T),
    median2021 = median(n_2021,na.rm = T)
  )
```
...
**Not available. Some of the data are NAs**
...
**There are very large values. Outliers exist**
